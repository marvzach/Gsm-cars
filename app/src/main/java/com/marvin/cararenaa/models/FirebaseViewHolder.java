...
public void bindCompanies(Carzarena mCarzarena){
    TextView nameTextView = mView.findViewById(R.id.carNameTextView);
    nameTextView.setText(mCarzarena.getWebsite());
    TextView publication = mView.findViewById(R.id.websiteTextView);
    publication.setText("Website:"+" "+mCarzarena.getWebsite());

}    @Override
    public void onClick(View v) {
        final ArrayList<Carzarena> restaurants = new ArrayList<>();
        ...
        ref.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for(DataSnapshot snapshot: dataSnapshot.getChildren()){
                    restaurants.add(snapshot.getValue(Carzarena.class));
                }

                int itemPosition = getLayoutPosition();
                Intent intent = new Intent(mContext, CararenaDetailActivity.class);
                intent.putExtra("position", itemPosition + "");
                intent.putExtra("carzarenas", Parcels.wrap(restaurants));
